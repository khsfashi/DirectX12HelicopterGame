...
void CFirstPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	if (m_pPlayer)
	{
		...
		XMFLOAT3 offset = XMFLOAT3(0.0f, 4.7f, 3.5f);
		...
		if (fDistance > 0)
		{
			...
			if (m_xmf3OldPosition.x && m_xmf3OldPosition.y && m_xmf3OldPosition.z) m_xmf3OldPosition = Vector3::Add(m_xmf3Position, xmf3Direction, fDistance);

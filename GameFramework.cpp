...
void CGameFramework::BuildObjects()
{
	...
	IDWriteTextFormat* pdwTextFormat = m_pUILayer->CreateTextFormat(L"Koverwatch", m_nWndClientHeight / 15.0f);
	D2D1_RECT_F d2dRect = D2D1::RectF(20.0f, m_nWndClientHeight - 100.0f, 80.0f, m_nWndClientHeight - 20.0f);

	WCHAR pstrOutputText[256];
	wcscpy_s(pstrOutputText, 256, L"HP\n");
	...
	pdwTextFormat = m_pUILayer->CreateTextFormat(L"Koverwatch", m_nWndClientHeight / 20.0f);
	d2dRect = D2D1::RectF((float)m_nWndClientWidth * 0.865f, m_nWndClientHeight * 0.9f, (float)m_nWndClientWidth, (float)m_nWndClientHeight * 0.98f);
...
{
	int MaxHp = m_pPlayer->GetMaxHp();
	int CurHp = m_pPlayer->GetCurHp();
	int MaxBullet = m_pPlayer->GetMaxBullet();
	int CurBullet = m_pPlayer->GetCurBullet();

	std::wstring bulletStr = std::to_wstring(CurBullet);
	bulletStr.insert(bulletStr.length(), L"/");
	bulletStr.insert(bulletStr.length(), std::to_wstring(MaxBullet));

	WCHAR* bulletWchar = (wchar_t*)bulletStr.c_str();

	m_pUILayer->UpdateTextOutputs(1, bulletWchar, NULL, NULL, NULL);

	m_pUILayer->SetPlayerHp(MaxHp, CurHp);

	for (int i = 0; i < MAX_ENEMY_COUNT; ++i) 
	{
		m_pUILayer->m_xmf2EnemyPos[i] = m_pScene->CalculObjectPlayerDistance(i);
	}
